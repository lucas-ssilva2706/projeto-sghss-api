...
public class ConsultaController {
	...
	public ResponseEntity<ConsultaResponseDTO> findById(@PathVariable Long id) {
		return consultaService.buscarPorId(id).map(record -> ResponseEntity.ok().body(record))
				.orElse(ResponseEntity.notFound().build());
	}

	@PostMapping
	@PreAuthorize("hasAnyAuthority('ADMIN', 'MEDICO', 'PACIENTE')")
	public ConsultaResponseDTO create(@Valid @RequestBody ConsultaDTO consultaDTO) {
		return consultaService.criarConsulta(consultaDTO);
	}

	@PatchMapping("/{id}/realizar")
	@PreAuthorize("hasAnyAuthority('MEDICO', 'ADMIN')")
	public ResponseEntity<ConsultaResponseDTO> realizarConsulta(@PathVariable Long id) {
		return consultaService.realizarConsulta(id).map(consulta -> ResponseEntity.ok().body(consulta))
				.orElse(ResponseEntity.notFound().build());
	}
	...
	public ResponseEntity<ConsultaResponseDTO> cancelarConsulta(@PathVariable Long id) {
		return consultaService.cancelarConsulta(id).map(consulta -> ResponseEntity.ok().body(consulta))
				.orElse(ResponseEntity.notFound().build());
	}

	@PatchMapping("/{id}/faltar")
	@PreAuthorize("hasAnyAuthority('MEDICO', 'ADMIN')")
	public ResponseEntity<ConsultaResponseDTO> informarFaltaConsulta(@PathVariable Long id) {
		return consultaService.informarFaltaConsulta(id).map(consulta -> ResponseEntity.ok().body(consulta))
				.orElse(ResponseEntity.notFound().build());
	}
}