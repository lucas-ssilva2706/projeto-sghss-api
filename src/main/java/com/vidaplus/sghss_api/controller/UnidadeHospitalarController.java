...
public class UnidadeHospitalarController {
    ...
    public UnidadeHospitalarController(UnidadeHospitalarService unidadeService) {
        this.unidadeHospitalarService = unidadeService;
    }

    @GetMapping
    public List<UnidadeHospitalar> findAll() {
        return unidadeHospitalarService.listarTodas();
    }

    @GetMapping(path = "/{id}")
    public ResponseEntity<UnidadeHospitalar> findById(@PathVariable long id) {
        return unidadeHospitalarService.buscarPorId(id)
            .map(record -> ResponseEntity.ok().body(record))
            .orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public UnidadeHospitalar create(@Valid @RequestBody UnidadeHospitalar unidadeHospitalar) {
        return unidadeHospitalarService.criarUnidade(unidadeHospitalar);
    }

    @PutMapping(value = "/{id}")
    public ResponseEntity<UnidadeHospitalar> update(@PathVariable long id, @Valid @RequestBody UnidadeHospitalar unidadeHospitalar) {
        return unidadeHospitalarService.atualizarUnidade(id, unidadeHospitalar)
    ...
    public ResponseEntity<?> delete(@PathVariable long id) {
        if (unidadeHospitalarService.deletarUnidade(id)) {
